...
export default function Pipeline() {
  ...
  const BOTTOM_ROW_NAMES = ["Visita", "Proposta", "Fechado"];
  const row1Columns = columns.filter(
    (col) => col.key === "none" || !BOTTOM_ROW_NAMES.includes(col.label)
  );
  const row2Columns = columns.filter(
    (col) => col.key !== "none" && BOTTOM_ROW_NAMES.includes(col.label)
  );
  ...
  return (
    <div style={{ width: "100%", minWidth: 0 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1rem", width: "100%", minWidth: 0 }}>
        {[row1Columns, row2Columns].map((rowCols, rowIndex) => (
          rowCols.length === 0 ? null : (
            <div key={rowIndex} style={{ width: "100%", minWidth: 0 }}>
              {rowIndex === 1 && (
                <p style={{ margin: "0 0 0.5rem", fontSize: "0.9rem", color: "#666", fontWeight: 600 }}>Visita → Proposta → Fechado</p>
              )}
              <div
                style={{
                  width: "100%",
              ...
              >
                <div style={{ display: "flex", gap: "1rem", minHeight: 280, width: "max-content" }}>
                  {rowCols.map((col) => (
                    <div
                      key={col.key}
