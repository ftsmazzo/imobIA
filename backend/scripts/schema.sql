...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_contacts_pipeline_stage') THEN
    ALTER TABLE contacts ADD CONSTRAINT fk_contacts_pipeline_stage FOREIGN KEY (pipeline_stage_id) REFERENCES pipeline_stages(id) ON DELETE SET NULL;
  END IF;
END $$;
